...
class DirectGroqLLM(BaseLLM):
    """CrewAI text/ReAct adapter using Groq directly, without LiteLLM."""

    def __init__(self, api_key):
        super().__init__(model='openai/gpt-oss-120b', temperature=0.2)
        self._api_key = api_key

    def supports_function_calling(self):
        # CrewAI uses its text-based tool protocol instead of native tool calls.
        return False
    ...
    def call(self, messages, tools=None, callbacks=None,
        ...
        for message in messages:
            ...
            if role not in ('system', 'user', 'assistant'):
                raise ValueError('Unexpected message role in text-based Groq adapter: ' + str(role))
            content = message.get('content') or ''
            if isinstance(content, list):
                content = '\n'.join(
                    str(part.get('text', '')) for part in content
                    if isinstance(part, dict) and part.get('type') == 'text')
            # Allowlist only role/content, removing cache_breakpoint and other metadata.
            clean.append({'role': role, 'content': str(content)})
        payload = {'model': self.model, 'messages': clean,
                   'temperature': self.temperature, 'max_completion_tokens': 4096}
        request = Request(
            'https://api.groq.com/openai/v1/chat/completions',
            data=json.dumps(payload).encode('utf-8'),
            headers={'Authorization': 'Bearer ' + self._api_key,
                     'Content-Type': 'application/json',
                     'User-Agent': 'AI-Research-Agent/1.1'}, method='POST')
        try:
            ...
            try:
                ...
                detail = 'Request rejected by Groq'
            raise RuntimeError(f'Groq HTTP {exc.code}: {detail}') from None
        except URLError as exc:
            raise RuntimeError(f'Groq connection failed: {exc.reason}') from None
        try:
            choice = result['choices'][0]
            text = choice['message']['content']
        except (KeyError, IndexError, TypeError):
            raise RuntimeError('Groq returned an unexpected response format.') from None
        if not isinstance(text, str) or not text.strip():
            raise RuntimeError('Groq returned no answer text. Retry with a shorter report.')
        if choice.get('finish_reason') == 'length':
            raise RuntimeError('Groq reached the output token limit. Retry with a shorter report.')
        # Apply CrewAI stop markers locally; never forward framework-specific metadata.
        stops = getattr(self, 'stop', None) or []
...
def research(topic, api_key, length):
    ...
    llm = DirectGroqLLM(api_key=api_key)
    agent = Agent(
